...
def initialize_logging():
    ...
    log_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'anki_addon.log')
    file_handler = logging.FileHandler(log_path, 'a', encoding='utf-8')

    file_handler.setLevel(logging.DEBUG)
    formatter = AnkiAddonLogger.LevelColoredFormatter("%(asctime)s %(combined_info)-20s [%(levelname)s] %(message)s")
